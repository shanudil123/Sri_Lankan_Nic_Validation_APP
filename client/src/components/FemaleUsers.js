...
const FemaleUsers = () => {
  ...
  useEffect(() => {
    const fetchFemaleUsers = async () => {
      try {
        ...
        const filteredFemaleUsers = response.data.filter(
          (user) => user.gender === "Female"
        );
        setFemaleUsers(filteredFemaleUsers);
      } catch (error) {
  ...
  const downloadPDF = () => {
    const doc = new jsPDF();
    doc.text("Female Users Report", 20, 10);
    doc.autoTable({
      head: [["NIC", "Birthday", "Age", "Gender"]],
      body: femaleUsers.map((user) => [
        user.nic,
        user.birthday,
        user.age,
        user.gender,
    ...
    doc.save("female_users_report.pdf");
  };

  const downloadExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(femaleUsers);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Female Users");
    XLSX.writeFile(workbook, "female_users_report.xlsx");
  };
    ...
    <div className="relative min-h-screen bg-gradient-to-br from-gray-800 via-gray-900 to-black py-8 px-6 flex justify-center items-center">
      ...
      <div className="bg-white rounded-lg shadow-lg p-8 w-full max-w-5xl">
        ...
        <div className="flex justify-center mt-8 gap-4">
          ...
          <CSVLink
            data={femaleUsers}
            filename={"female_users_report.csv"}
